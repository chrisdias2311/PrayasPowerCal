...
@Component({
  selector: 'app-calorific',
  templateUrl: './calorific.component.html',
  styleUrls: ['./calorific.component.css']
})
export class CalorificComponent implements OnInit {
  Form = new FormGroup({
  ...
  calculate(car:string,hyd:string,oxy:string,sul:string) {
    ...
    if((this.hcv>8500 && parseFloat(hyd)>5) || (this.hcv>8500 && parseFloat(oxy)>=1) || (this.hcv>8500 && parseFloat(sul)>=0.5)){
      this.cornerCase=true;
    } else if(this.hcv>8500){
      this.efficientFuel=true;
    } else if((parseFloat(car)>=75)&&(parseFloat(hyd)<=5)&&(parseFloat(oxy)<=1)&&parseFloat(sul)<=0.5){
      this.efficientFuel=true;
    } else {
    ...
    // if((parseFloat(car)>=75)&&(parseFloat(hyd)<=5)&&(parseFloat(oxy)<=1)&&parseFloat(sul)<=0.5){
    //   this.efficientFuel=true;
    // } else if(this.hcv>8500 && parseFloat(hyd)>=5){
    //   this.cornerCase=true;
    // } else {
    //   this.notEfficientFuel=true;
    // }
